...
async function handleSetupComplete(event: MessageEvent): Promise<void> {
  ...
  if (event.data?.type === 'kms:request-push-subscription') {
    ...
    try {
      ...
      // Unsubscribe any existing subscription to avoid conflicts
      const existingSub = await registration.pushManager.getSubscription();
      if (existingSub) {
        console.log('[Full Demo] Unsubscribing existing push subscription');
  ...
  if (event.data?.type === 'kms:send-test-notification') {
    ...
    try {
      // Send mock push notification via service worker (avoids CSP violation)
      const registration = await navigator.serviceWorker.ready;
      ...
      await pushPromise;
      console.log('[Full Demo] Test notification sent successfully');
...
// Legacy setupPassphrase() and setupWebAuthn() functions removed.
// Use setupWithPopupKMSOnly() for the new KMS-only popup flow.
