...
describe('iframe MessagePort transport (BUG-014)', () => {
  let env: ReturnType<typeof setupTestEnvironment>;
  let client: KMSClient;
  ...
  /**
   * Drive the kms:connect handshake from the parent side: dispatch a window message that
   * transfers one end of a MessageChannel, and resolve once the enclave acks kms:connected
   * on the parent-held port. Returns the parent-held port for sending/receiving RPC traffic.
   */
  function connectPort(origin: string = 'https://allthe.services'): {
    port: MessagePort;
    ...
    const connected = new Promise((resolve) => {
      channel.port1.onmessage = (e: MessageEvent): void => {
        if ((e.data as { type?: string })?.type === 'kms:connected') resolve(e.data);
      };
    });
    const event = new MessageEvent('message', {
      data: { type: 'kms:connect' },
      origin,
      ports: [channel.port2],
    });
    window.dispatchEvent(event);
  ...
  it('establishes the port and acknowledges kms:connected', async () => {
    const { connected } = connectPort();
    const ack = await connected;
    expect(ack).toEqual({ type: 'kms:connected' });
  });

  it('ignores a kms:connect from an invalid origin (origin check precedes the handshake)', async () => {
    const channel = new MessageChannel();
    let acked = false;
  ...
  it('forwards a request received over the port to the worker', async () => {
    const { port, connected } = connectPort();
    await connected;
  ...
  it('routes RPC responses over the port, not the window, once connected', async () => {
    const { port, connected } = connectPort();
    await connected;
    ...
    const worker = env.getMockWorker();
    const response: RPCResponse = { id: 'abc-1', result: { ok: true } };
    env.mockParent.clearMessages();
    worker!.simulateMessage(response);
    await new Promise((r) => setTimeout(r, 20));

    // Delivered over the port…
    expect(received).toContainEqual(response);
    // …and NOT over the window channel.
    expect(env.mockParent.receivedMessages).toHaveLength(0);
  });

  it('keeps typed control messages on the window channel even when the port is connected', async () => {
    const { port, connected } = connectPort();
    await connected;

    const portMsgs: unknown[] = [];
    port.onmessage = (e: MessageEvent): void => {
      portMsgs.push(e.data);
    };

    const worker = env.getMockWorker();
    env.mockParent.clearMessages();
    // A typed (non-RPC) worker message falls through to the catch-all and must go to window.
    worker!.simulateMessage({ type: 'kms:control-probe' });
    await new Promise((r) => setTimeout(r, 20));

    expect(env.mockParent.getLastMessage()?.data).toEqual({ type: 'kms:control-probe' });
    // The control message must NOT have been delivered over the port.
    expect(portMsgs).not.toContainEqual({ type: 'kms:control-probe' });
  });
});
